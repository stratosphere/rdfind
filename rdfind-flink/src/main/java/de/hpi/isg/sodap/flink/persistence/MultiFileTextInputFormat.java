 ...
 **********************************************************************************************************************/
package de.hpi.isg.sodap.flink.persistence;

import de.hpi.isg.sodap.flink.compression.InflaterInputStreamFactory;
import de.hpi.isg.sodap.flink.compression.InputStreamFSInputWrapper;
import de.hpi.isg.sodap.flink.util.Encoding;
...
public class MultiFileTextInputFormat extends DelimitedInputFormat<Tuple2<Integer, String>>
    ...
    public static MultiFileTextInputFormat createFor(String... inputPaths) {
        ...
        MultiFileTextInputFormat inputFormat;
        inputFormat = new MultiFileTextInputFormat(fileIdRetriever, fileIdRetriever);
        String inputPath = FileUtils.findCommonParentPath2(Arrays.asList(inputPaths));
        inputFormat.setFilePath(inputPath);
    ...
    /** Used to map an {@link de.hpi.isg.sodap.flink.util.Encoding} to each file. */
    ...
    // This went as contribution into Flink.
//    @Override
//    protected FSDataInputStream decorateInputStream(FSDataInputStream stream, FileInputSplit split) throws Throwable {
//        stream = super.decorateInputStream(stream, split);
//
//        // See if we have to decompress the split.
//        InflaterInputStreamFactory<?> inflaterInputStreamFactory =
//                FileUtils.findInflaterInputStreamFactory(split.getPath());
//        if (inflaterInputStreamFactory != null) {
//            stream = new InputStreamFSInputWrapper(inflaterInputStreamFactory.create(stream));
//        }
//
//        if (split.getStart() == 0) {
//            Encoding.ByteOrderMark bom = this.encoding.getByteOrderMark();
//            LOGGER.debug("Skipping {} in {} on {}.", bom, split, stream);
//            try {
//                bom.skip(stream);
//            } catch (Exception e) {
//                throw new IOException(String.format("Skipping the BOM %s in %s failed.", bom, split), e);
//            }
//        }
//
//        return stream;
//    }
    ...
    /**
     * Instead of specifyin an encoding per file, this setter allows to set the same encoding for all files.
     */
